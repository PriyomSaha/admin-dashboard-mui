...
function TableActionMenu({
  ...
  useEffect(() => {
    ...
    if (menuButtonElement != null && dropDownElement != null) {
      ...
      // Set the bottom of the dropdown to be fixed to the bottom of the window
      dropDownElement.style.bottom = "0";

      // Calculate the top position to center the dropdown vertically
      const centerTop = window.innerHeight / 2 - menuButtonRect.height / 2;
      dropDownElement.style.top = `${centerTop - 50}px`;

      // Calculate the left position
      const leftPosition =
        menuButtonRect.left + menuButtonRect.width + window.pageXOffset + 5;

      // Check if the dropdown goes out of the screen on the right
      if (leftPosition + dropDownElement.offsetWidth > window.innerWidth) {
        // If it does, place it to the left of the cell
        dropDownElement.style.left = `${
          menuButtonRect.left - dropDownElement.offsetWidth - 5
        }px`;
      } else {
        // Otherwise, place it to the right of the cell
